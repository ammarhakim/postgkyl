...
def get_p_par(p_data=None, p_grid=None, p_values=None,
    ...
    out = (b_x*b_x*p_xx + b_y*b_y*p_yy + b_z*b_z*p_zz + 2.0*(b_x*b_y*p_xy + b_x*b_z*p_xz + b_y*b_z*p_yz))/mag_b_sq
    return grid, out
#end
...
def get_p_perp(p_data=None, p_grid=None, p_values=None,
    ...
    grid, p_par = get_p_par(p_grid=p_grid, p_values=p_values, b_grid=b_grid, b_values=b_values) 
...
def get_agyro(p_data=None, p_grid=None, p_values=None,
              b_data=None, b_grid=None, b_values=None,
              measure="swisdak"):
    if p_data:
        p_grid = p_data.getGrid()
    ...
    grid, mag_b_sq = diag.mag_sq(in_grid=b_grid, in_values=b_values)
    _, p_par = get_p_par(p_grid=p_grid, p_values=p_values, b_grid=b_grid, b_values=b_values)
    _, p_perp = get_p_perp(p_grid=p_grid, p_values=p_values, b_grid=b_grid, b_values=b_values)

    if measure.lower() == "swisdak":
        I1 = p_xx + p_yy + p_zz
        I2 = p_xx*p_yy + p_xx*p_zz + p_yy*p_zz - (p_xy*p_xy + p_xz*p_xz + p_yz*p_yz)
        ...
        out = np.sqrt(p_ixx**2 + 2*p_ixy**2 + 2*p_ixz**2 + p_iyy**2 + 2*p_iyz**2 + p_izz**2) \
...
def get_gkyl_10m_agyro(species_data=None, species_grid=None, species_values=None,
                       field_data=None, field_grid=None, field_values=None,
                       measure="swisdak"):
    if species_data:
        species_grid = species_data.getGrid()
    ...
    return get_agyro(p_grid=p_grid, p_values=p_values, b_grid=b_grid, b_values=b_values, measure=measure)
#end
