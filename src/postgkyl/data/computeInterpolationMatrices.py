...
def createInterpMatrix(dim, order, basis_type, interp, modal=True, c2p=False, num_cdim=0):
  if c2p:
    interp += 1
  # end
  ...
  for d in range(dim):
    ...
    if basis_type == "gkhybrid":
      ...
      # Hybrid basis has p=2 in each velocity space dim.
      if d >= num_cdim:
        interp_true = interp + 1
      # end
    # end
      ...
      if order == 1:
        if num_cdim == 1:
            functionVector = Matrix(
                ...
                [1.837117307087383*x*y*z],
                [1.185854122563142*(y**2-0.3333333333333333)],
                [2.0539595906443724*(x*y**2-0.3333333333333333*x)],
                [2.0539595906443724*(y**2*z-0.3333333333333333*z)],
                [3.5575623676894255*(x*y**2*z-0.3333333333333333*x*z)],
                [1.185854122563142*(z**2-0.3333333333333333)],
                [2.0539595906443724*(x*z**2-0.3333333333333333*x)],
                [2.0539595906443724*(y*z**2-0.3333333333333333*y)],
                [3.5575623676894255*(x*y*z**2-0.3333333333333333*x*y)],
                ]
            )
        elif num_cdim == 2:
            functionVector = Matrix(
            ...
            raise NameError(
                "interpMatrix: Dimensionality incorrect! Total dimensions: {}\nConfiguration space dimensions: {}".format(
                    dim, num_cdim
                )
            )
        interpMatrix = numpy.zeros(
        ...
        if num_cdim == 1:
            functionVector = Matrix(
                [
                    ...
                    [0.75*w*y],
                    [0.75*w*z],
                    [1.2990381056766578*x*y*z],
                    [1.2990381056766578*w*x*y],
                    [1.2990381056766578*w*x*z],
                    [1.2990381056766578*w*y*z],
                    [2.25*w*x*y*z],
                    [0.8385254915624212*(y**2-0.3333333333333333)],
                    [1.4523687548277815*(x*y**2-0.3333333333333333*x)],
                    [1.4523687548277815*(y**2*z-0.3333333333333333*z)],
                    [1.4523687548277815*(w*y**2-0.3333333333333333*w)],
                    [2.5155764746872635*(x*y**2*z-0.3333333333333333*x*z)],
                    [2.5155764746872635*(w*x*y**2-0.3333333333333333*w*x)],
                    [2.5155764746872635*(w*y**2*z-0.3333333333333333*w*z)],
                    [4.3571062644833445*(w*x*y**2*z-0.3333333333333333*w*x*z)],
                    [0.8385254915624212*(z**2-0.3333333333333333)],
                    [1.4523687548277815*(x*z**2-0.3333333333333333*x)],
                    [1.4523687548277815*(y*z**2-0.3333333333333333*y)],
                    [1.4523687548277815*(w*z**2-0.3333333333333333*w)],
                    [2.5155764746872635*(x*y*z**2-0.3333333333333333*x*y)],
                    [2.5155764746872635*(w*x*z**2-0.3333333333333333*w*x)],
                    [2.5155764746872635*(w*y*z**2-0.3333333333333333*w*y)],
                    [4.3571062644833445*(w*x*y*z**2-0.3333333333333333*w*x*y)],
                    [0.8385254915624212*(w**2-0.3333333333333333)],
                    [1.4523687548277815*(w**2*x-0.3333333333333333*x)],
                    [1.4523687548277815*(w**2*y-0.3333333333333333*y)],
                    [1.4523687548277815*(w**2*z-0.3333333333333333*z)],
                    [2.5155764746872635*(w**2*x*y-0.3333333333333333*x*y)],
                    [2.5155764746872635*(w**2*x*z-0.3333333333333333*x*z)],
                    [2.5155764746872635*(w**2*y*z-0.3333333333333333*y*z)],
                    [4.3571062644833445*(w**2*x*y*z-0.3333333333333333*x*y*z)],
                ]
            )
        elif num_cdim == 2:
            functionVector = Matrix(
      ...
      if order == 1:
        if num_cdim == 2:
                functionVector = Matrix(
                    ...
                    [0.9185586535436913*v*w*y],
                    [0.9185586535436913*v*w*z],
                    [1.5909902576697312*w*x*y*z],
                    [1.5909902576697312*v*x*y*z],
                    [1.5909902576697312*v*w*x*y],
                    [1.5909902576697312*v*w*x*z],
                    [1.5909902576697312*v*w*y*z],
                    [2.7556759606310735*v*w*x*y*z],
                    [0.592927061281571*(z**2-0.3333333333333333)],
                    [1.0269797953221862*(x*z**2-0.3333333333333333*x)],
                    [1.0269797953221862*(y*z**2-0.3333333333333333*y)],
                    [1.0269797953221862*(w*z**2-0.3333333333333333*w)],
                    [1.0269797953221862*(v*z**2-0.3333333333333333*v)],
                    [1.7787811838447127*(x*y*z**2-0.3333333333333333*x*y)],
                    [1.7787811838447127*(w*x*z**2-0.3333333333333333*w*x)],
                    [1.7787811838447127*(w*y*z**2-0.3333333333333333*w*y)],
                    [1.7787811838447127*(v*x*z**2-0.3333333333333333*v*x)],
                    [1.7787811838447127*(v*y*z**2-0.3333333333333333*v*y)],
                    [1.7787811838447127*(v*w*z**2-0.3333333333333333*v*w)],
                    [3.080939385966558*(w*x*y*z**2-0.3333333333333333*w*x*y)],
                    [3.080939385966558*(v*x*y*z**2-0.3333333333333333*v*x*y)],
                    [3.080939385966558*(v*w*x*z**2-0.3333333333333333*v*w*x)],
                    [3.080939385966558*(v*w*y*z**2-0.3333333333333333*v*w*y)],
                    [5.336343551534138*(v*w*x*y*z**2-0.3333333333333333*v*w*x*y)],
                    [0.592927061281571*(w**2-0.3333333333333333)],
                    [1.0269797953221862*(w**2*x-0.3333333333333333*x)],
                    [1.0269797953221862*(w**2*y-0.3333333333333333*y)],
                    [1.0269797953221862*(w**2*z-0.3333333333333333*z)],
                    [1.0269797953221862*(v*w**2-0.3333333333333333*v)],
                    [1.7787811838447127*(w**2*x*y-0.3333333333333333*x*y)],
                    [1.7787811838447127*(w**2*x*z-0.3333333333333333*x*z)],
                    [1.7787811838447127*(w**2*y*z-0.3333333333333333*y*z)],
                    [1.7787811838447127*(v*w**2*x-0.3333333333333333*v*x)],
                    [1.7787811838447127*(v*w**2*y-0.3333333333333333*v*y)],
                    [1.7787811838447127*(v*w**2*z-0.3333333333333333*v*z)],
                    [3.080939385966558*(w**2*x*y*z-0.3333333333333333*x*y*z)],
                    [3.080939385966558*(v*w**2*x*y-0.3333333333333333*v*x*y)],
                    [3.080939385966558*(v*w**2*x*z-0.3333333333333333*v*x*z)],
                    [3.080939385966558*(v*w**2*y*z-0.3333333333333333*v*y*z)],
                    [5.336343551534138*(v*w**2*x*y*z-0.3333333333333333*v*x*y*z)],
                    [0.592927061281571*(v**2-0.3333333333333333)],
                    [1.0269797953221862*(v**2*x-0.3333333333333333*x)],
                    [1.0269797953221862*(v**2*y-0.3333333333333333*y)],
                    [1.0269797953221862*(v**2*z-0.3333333333333333*z)],
                    [1.0269797953221862*(v**2*w-0.3333333333333333*w)],
                    [1.7787811838447127*(v**2*x*y-0.3333333333333333*x*y)],
                    [1.7787811838447127*(v**2*x*z-0.3333333333333333*x*z)],
                    [1.7787811838447127*(v**2*y*z-0.3333333333333333*y*z)],
                    [1.7787811838447127*(v**2*w*x-0.3333333333333333*w*x)],
                    [1.7787811838447127*(v**2*w*y-0.3333333333333333*w*y)],
                    [1.7787811838447127*(v**2*w*z-0.3333333333333333*w*z)],
                    [3.080939385966558*(v**2*x*y*z-0.3333333333333333*x*y*z)],
                    [3.080939385966558*(v**2*w*x*y-0.3333333333333333*w*x*y)],
                    [3.080939385966558*(v**2*w*x*z-0.3333333333333333*w*x*z)],
                    [3.080939385966558*(v**2*w*y*z-0.3333333333333333*w*y*z)],
                    [5.336343551534138*(v**2*w*x*y*z-0.3333333333333333*w*x*y*z)],
                    ]
                )
        elif num_cdim == 3:
                functionVector = Matrix(
