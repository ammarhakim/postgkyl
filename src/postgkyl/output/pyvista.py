...
def _centered_grid_3d(
    grid: list[np.ndarray], value_shape: tuple[int, int, int]
...
def pyvista(
    ...
    **kwargs,
    ...
    x, y, z, scalar = _downsample_3d_volume(
        x, y, z, scalar, maximum_points_per_axis=max_points_per_axis
    )
    ...
    if is_log:
        ...
        cmin, cmax = (
            np.log10(cmin) if cmin is not None else None,
            np.log10(cmax) if cmax is not None else None,
        )
    # end
    grid3d["f_plot"] = data

    clim = (
        cmin if cmin is not None else datamin,
    ...
    if spin:
        ...
        pl.add_timer_event(
            max_steps=99999999, duration=50, callback=rotate_callback
        )  # 20 FPS
    ...
    if saveas != "":
        if saveas.endswith(".html"):
            ...
            saveas.endswith(".png")
            or saveas.endswith(".jpg")
            or saveas.endswith(".jpeg")
        ):
            ...
            raise ValueError(
                "Unsupported file format for saving. Supported formats are: .html, .png, .jpg, .jpeg, .pdf, .svg"
            )
