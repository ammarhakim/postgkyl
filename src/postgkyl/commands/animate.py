...
@click.option(
    "--squeeze", "-s", is_flag=True, help="Squeeze the components into one panel."
)
@click.option(
...
@click.option(
    "--sdensity", type=click.FLOAT, help="Control density of the streamlines."
)
@click.option(
    "--arrowstyle", type=click.STRING, help="Set the style for streamline arrows."
)
@click.option(
    "-g", "--group", type=click.Choice(["0", "1"]), help="Switch to group mode."
)
...
@click.option(
    "-d", "--diverging", is_flag=True, help="Switch to diverging colormesh mode."
)
@click.option(
    "--arg", type=click.STRING, help="Additional plotting arguments, e.g., '*--'."
)
@click.option(
    "-a",
    "--fix-aspect",
    ...
    for tag in tag_iterator:
      ...
      if not kwargs["saveframes"]:
        ...
        if kwargs["save"] or kwargs["saveas"]:
          anims[-1].save(
              file_name, writer="ffmpeg", fps=kwargs["fps"], dpi=kwargs["dpi"]
          )
        # end
      else:
