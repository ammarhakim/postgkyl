...
@click.option(
    "--read", "-r", type=click.BOOL, help="Read from general interpolation file"
)
...
def differentiate(ctx, **kwargs):
  ...
  for dat in data.iterator(kwargs["use"]):
    if kwargs["basis_type"] is None and dat.ctx["basis_type"] is None:
      ctx.fail(
          click.style(
              "ERROR in interpolate: no 'basis_type' was specified and dataset {:s} does not have required ctxdata".format(
                  dat.get_label()
              ),
              fg="red",
          )
    ...
    if is_modal or dat.ctx["is_modal"]:
      ...
      dg = GInterpNodal(
          dat, kwargs["poly_order"], basis_type, kwargs["interp"], kwargs["read"]
      )
    # end
