...
@click.option(
    "--momentum", "-m", default="momentum", show_default=True, help="Tag for momentum"
)
@click.option(
    "--tag", "-t", default="velocity", show_default=True, help="Tag for the result"
)
@click.option(
    "--label",
    "-l",
...
def velocity(ctx, **kwargs):
  ...
  for m0, m1 in zip(
      data.iterator(kwargs["density"]), data.iterator(kwargs["momentum"])
  ):
    grid = m0.get_grid()
