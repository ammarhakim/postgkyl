  ...
  for setIdx in range(max(numSets)):
    ...
    for i in range(numIn):
      for key in tmp_ctx[i]:
        if key in out_ctx and _compare(
            tmp_ctx[i][key], out_ctx[key]
        ):  # tmp_ctx[i][k] == out_ctx[k]:
...
def ev(ctx, **kwargs):
  ...
  for s in chainSplit:
    isData, dataId = _data(
        ctx, grid_stack, value_stack, ctx_stack, s, tags, only_active
    )
  ...
  if len(value_stack) == 0:
    ctx.fail(
        click.style("Evaluate stack is empty, there is nothing to return", fg="red")
    )
  elif len(value_stack) > 1:
    click.echo(
        click.style(
            "WARNING: Length of the evaluate stack is bigger than 1, there is a posibility of unintended behavior",
            fg="yellow",
        )
    )
  # end
  if numDatasetsInChain == 1 and kwargs["tag"] is None:
    cnt = 0
    tag = outDataId[0]
    for out in ctx.obj["data"].iterator(
        tag=tag, select=outDataId[1], only_active=only_active
    ):
