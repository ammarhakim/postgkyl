...
@click.option(
    "--bfield", "-b", default="field", show_default=True, help="Tag for input EM field."
)
...
def agyro(ctx, **kwargs):
  ...
  for pressure, bfield in zip(
      data.iterator(kwargs["pressure"]), data.iterator(kwargs["bfield"])
  ):
    grid, agyro = get_agyro(p_data=pressure, b_data=bfield, measure=kwargs["measure"])
    out = GData(
        tag=tag, label=kwargs["label"], comp_grid=ctx.obj["compgrid"], ctx=pressure.ctx
    )
    out.push(grid, agyro)
    data.add(out)
  # end
...
def mom_agyro(ctx, **kwargs):
  ...
  for species, field in zip(
      data.iterator(kwargs["species"]), data.iterator(kwargs["field"])
  ):
    grid, agyro = get_gkyl_10m_agyro(
        species_data=species, field_data=field, measure=kwargs["measure"]
    )
    out = GData(
        tag=tag, label=kwargs["label"], comp_grid=ctx.obj["compgrid"], ctx=species.ctx
    )
    out.push(grid, agyro)
    data.add(out)
  # end
